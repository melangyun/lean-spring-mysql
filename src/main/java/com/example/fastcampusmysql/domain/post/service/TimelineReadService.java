...
public class TimelineReadService {
    ...
    public PageCursor<Timeline> getTimelines(Long memberId, CursorRequest cursorRequest) {
        ...
        long nextKey = timelines.stream().mapToLong(Timeline::getId).min().orElse(0);
    ...
    private List<Timeline> findAllBy(Long memberId, CursorRequest cursorRequest) {
        if (cursorRequest.hasKey()) {
            return timelineRepository.findAllByLessThanIdAndMemberIdOrderByIdDesc(cursorRequest.key(), memberId, cursorRequest.size());
        }
